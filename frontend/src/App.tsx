...
const AcidSlider = ({ value, onChange, label, min = 0, max = 100, displayValue }: any) => (
  <div className="space-y-2">
    <div className="flex justify-between items-center">
      <span className="text-[10px] uppercase tracking-widest text-white/40 font-bold">{label}</span>
      <span className="text-xs font-mono text-acid">{displayValue ?? `${value}%`}</span>
    </div>
...
const DashboardView = ({ onDeviceClick }: { onDeviceClick: (type: ViewType) => void }) => {
  ...
  useEffect(() => {
    api.getDevices().then(data => {
      ...
      setLoading(false);
      data.forEach(d => {
        api.getDeviceState(d.id).then(state => {
          setDeviceStates(prev => ({ ...prev, [d.id]: state }));
        }).catch(() => {});
      });
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <GlassCard className="lg:col-span-2">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
          {[
            { label: 'CPU Temp', value: '42°C', sub: 'Last 1h avg: 81°' },
            { label: 'GPU Temp', value: '39°C', sub: 'Last 1h avg: 75°' },
...
const DeviceView = ({ type }: { type: string }) => {
  ...
  const matchingDevices = (devicesData as any[]).filter(d => d.category.toLowerCase() === type.toLowerCase());
  const [selectedDeviceIdx, setSelectedDeviceIdx] = useState(0);
  const [selectedHotspot, setSelectedHotspot] = useState<string | null>(null);
  const currentDevice = matchingDevices[Math.min(selectedDeviceIdx, matchingDevices.length - 1)] || null;
  ...
  const handleDpiChange = async (newDpi: number) => {
    ...
    if (deviceState) {
      try {
        ...
        console.error('Failed to set DPI:', e);
      }
    }
  };
  ...
  const handlePollRateChange = async (rate: string) => {
    ...
    if (deviceState) {
      try {
        ...
        console.error('Failed to set poll rate:', e);
      }
    }
  };
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-80 border-r border-border p-8 overflow-y-auto custom-scrollbar space-y-8 bg-charcoal/30">
          <AnimatePresence mode="wait">
            {activeSubTab === 'Performance' && (
              ...
              >
                <div className="space-y-6">
                  ...
                  <AcidSlider label="Dots Per Inch (DPI)" value={(dpi / 20000) * 100} onChange={(v: number) => handleDpiChange(Math.round((v / 100) * 20000))} displayValue={`${dpi.toLocaleString()} DPI`} />
                </div>
            ...
            {activeSubTab === 'Customize' && (
              ...
              >
                <div className="space-y-4">
                  <h3 className="text-xs font-bold uppercase tracking-widest text-white/60">Button Mapping</h3>
                  <p className="text-[11px] text-white/30 leading-relaxed">Click a hotspot on the device or select a button below.</p>
                  <div className="space-y-2">
                    {currentDevice?.hotspots?.map((h: any) => (
                      <button
                        key={h.id}
                        onClick={() => setSelectedHotspot(selectedHotspot === h.id ? null : h.id)}
                        className={`w-full text-left p-3 rounded-lg border transition-all text-xs font-bold ${
                          selectedHotspot === h.id
                            ? 'bg-acid/10 border-acid/30 text-acid'
                            : 'bg-surface border-border text-white/40 hover:border-white/20 hover:text-white/70'
                        }`}
                      >
                        {h.label}
                      </button>
        ...
        {/* Center Hero Panel */}
        <div className="flex-1 relative flex flex-col bg-[radial-gradient(circle_at_center,_rgba(0,255,65,0.03)_0%,_transparent_70%)] overflow-hidden">
          {currentDevice ? (
            <DeviceViewport
          ...
          {matchingDevices.length > 1 && (
            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex flex-wrap justify-center gap-2 px-4">
              {matchingDevices.map((d: any, i: number) => (
                <button
                  key={d.id}
                  onClick={() => { setSelectedDeviceIdx(i); setSelectedHotspot(null); }}
                  className={`px-3 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-widest transition-all border ${
                    selectedDeviceIdx === i
                      ? 'bg-acid/10 border-acid/30 text-acid'
                      : 'bg-surface/80 border-border text-white/40 hover:text-white backdrop-blur-sm'
                  }`}
                >
                  {(d.name as string).replace('Razer ', '')}
                </button>
              ))}
            </div>
          )}
        ...
        {/* Right Info Panel */}
        {activeSubTab === 'Customize' && currentDevice ? (
          <ConfigPanel device={currentDevice} activeTab="Customize" selectedHotspot={selectedHotspot} />
        ) : (
          <div className="w-80 border-l border-border p-8 overflow-y-auto custom-scrollbar space-y-8 bg-charcoal/30">
            <div className="glass-card p-6 space-y-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="text-xs font-bold uppercase tracking-wider">{deviceState?.name || currentDevice?.name || type}</div>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="text-sm font-display font-bold tracking-wide">
                {activeView === 'Dashboard' ? 'Overview' :
                  (() => {
                    const d = (devicesData as any[]).find(dev => dev.category.toLowerCase() === activeView.toLowerCase());
                    return d ? d.name.replace('Razer ', '') : activeView;
                  })()
                }
              </div>
            </div>
